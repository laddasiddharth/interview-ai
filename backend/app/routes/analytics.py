...
def get_user_analytics(
    ...
    answers = (
        db.query(Answer)
        .join(InterviewSession, Answer.interview_id == InterviewSession.id)
        .filter(InterviewSession.user_id == current_user.id, Answer.score.isnot(None))
        .all()
    )
    ...
    for interview_id, data in interview_scores.items():
        ...
        performance_data.append({"date": data["date"], "score": round(avg, 2)})
